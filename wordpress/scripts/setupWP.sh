...
W3TC_OPTION_SET="${WP} --path=web w3-total-cache option set"
LSCWP_OPTION_SET="${WP} --path=web litespeed-option set"
lOG="/var/log/run.log"
...
if [[ ${COMPUTE_TYPE} == *"llsmp"* || ${COMPUTE_TYPE} == *"litespeed"* ]] ; then
	${WP} --path=web plugin install litespeed-cache --activate --path=${SERVER_WEBROOT}
	CACHE_FLUSH="${WP} --path=web litespeed-purge all --path=${SERVER_WEBROOT}; rm -rf /var/www/webroot/.cache/vhosts/Jelastic/* "
        WPCACHE='lscwp';
elif [[ ${COMPUTE_TYPE} == *"lemp"* || ${COMPUTE_TYPE} == *"nginx"* ]] ; then
	${WP} --path=web plugin install w3-total-cache --activate --path=${SERVER_WEBROOT}
	CACHE_FLUSH="${WP} --path=web w3-total-cache flush all --path=${SERVER_WEBROOT}; /var/www/webroot/.cache/* "
        WPCACHE="w3tc";
else
...
if [ $WPCACHE == 'w3tc' ] ; then
	CDN_ENABLE_CMD="${WP} --path=web w3-total-cache option set cdn.enabled true --type=boolean"
elif [ $WPCACHE == 'lscwp' ] ; then
	CDN_ENABLE_CMD="${WP} --path=web litespeed-option set cdn true"
fi
...
${WP} --path=web cache flush --path=${SERVER_WEBROOT} &>> /var/log/run.log
crontab -l | sed "/checkCdnStatus/d" | crontab -
EOF
chmod +x ~/bin/checkCdnStatus.sh
PROTOCOL=$(${WP} --path=web option get siteurl --path=${SERVER_WEBROOT} | cut -d':' -f1)
...
if [ $edgeportCDN == 'true' ] ; then
  if ! $(${WP} --path=web core is-installed --network --path=${SERVER_WEBROOT}); then 
    case $WPCACHE in
      w3tc)
          checkCdnStatus;
      ...
      lscwp)
        checkCdnStatus;
        CDN_ORI=$(${WP} --path=web option get siteurl --path=${SERVER_WEBROOT} | cut -d'/' -f3)
        PROTOCOL=$(${WP} --path=web option get siteurl --path=${SERVER_WEBROOT} | cut -d':' -f1)
...
if [ $wpmu == 'true' ] ; then
  case $WPCACHE in
    w3tc)
          ${WP} --path=web plugin deactivate w3-total-cache --path=${SERVER_WEBROOT} &>> /var/log/run.log
	  [[ ${MODE} == 'subdir' ]] && ${WP} --path=web core multisite-convert --path=${SERVER_WEBROOT} &>> /var/log/run.log
	  [[ ${MODE} == 'subdom' ]] && ${WP} --path=web core multisite-convert --path=${SERVER_WEBROOT} --subdomains &>> /var/log/run.log
	  ${WP} --path=web plugin activate w3-total-cache --path=${SERVER_WEBROOT} &>> /var/log/run.log
          ;;
    lscwp)
          ${WP} --path=web plugin deactivate litespeed-cache --path=${SERVER_WEBROOT} &>> /var/log/run.log
          [[ ${MODE} == 'subdir' ]] && ${WP} --path=web core multisite-convert --path=${SERVER_WEBROOT} &>> /var/log/run.log
          [[ ${MODE} == 'subdom' ]] && ${WP} --path=web core multisite-convert --path=${SERVER_WEBROOT} --subdomains &>> /var/log/run.log
          ${WP} --path=web plugin activate litespeed-cache --path=${SERVER_WEBROOT} &>> /var/log/run.log
          ;;
  esac
fi
...
if [ $DOMAIN != 'false' ] ; then
  if ! $(${WP} --path=web core is-installed --network --path=${SERVER_WEBROOT}); then 
	OLD_DOMAIN=$(${WP} --path=web option get siteurl --path=${SERVER_WEBROOT})
	OLD_SHORT_DOMAIN=$(${WP} --path=web option get siteurl --path=${SERVER_WEBROOT} | cut -d'/' -f3)
	NEW_SHORT_DOMAIN=$(echo $DOMAIN | cut -d'/' -f3)

	${WP} --path=web search-replace "${OLD_DOMAIN}" "${DOMAIN}" --skip-columns=guid --all-tables --path=${SERVER_WEBROOT} &>> /var/log/run.log
	${WP} --path=web search-replace "${OLD_SHORT_DOMAIN}" "${NEW_SHORT_DOMAIN}" --skip-columns=guid --all-tables --path=${SERVER_WEBROOT} &>> /var/log/run.log
	${CACHE_FLUSH}  &>> /var/log/run.log
	${WP} --path=web cache flush --path=${SERVER_WEBROOT} &>> /var/log/run.log
  fi
fi
